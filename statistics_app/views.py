...
def is_instructor(user):
    ...
    return user.groups.filter(name='__instructor__').exists()
...
def team_stats(request, team_id=None):
    ...
    if not teams.exists():
        messages.add_message(
            request, messages.INFO, "No teams have been created yet. Please contact the administrator."
        )
    ...
    if team_id:
        try:
            ...
            messages.add_message(
                request, messages.ERROR, "Team not found"
            )
